"use client";

import Image from "next/image";
import { useState } from "react";

const slides = [
  {
    id: 1,
    name: "Air Zoom Pegasus 40",
    category: "Men's Running",
    price: 189,
    image: "/images/products/Tennis.png",
    season: "Men's",
    seasonAccent: "Running",
  },
  {
    id: 2,
    name: "Air Nike Tempo Next Nature",
    category: "Women's Running",
    price: 80,
    image: "/images/products/Tennis2.png",
    season: "Women's",
    seasonAccent: "Running",
  },
  {
    id: 3,
    name: "leggings Nike Pro",
    category: "Gym Clothes",
    price: 99,
    image: "/images/products/Ropagym.png",
    season: "Gym",
    seasonAccent: "Clothes",
  },
  {
    id: 4,
    name: "Nike Dri-FIT One",
    category: "Shorts Gym",
    price: 115,
    image: "/images/products/Shortsgym.png",
    season: "Shorts",
    seasonAccent: "Gym",
  },
];

export default function PromoSlider() {
  const [activeIndex, setActiveIndex] = useState(0);

  const active = slides[activeIndex];

  return (
    <section className="bg-[#F5F5F3] w-full relative overflow-hidden">
      <div className="absolute right-6 top-0 h-full z-0 pointer-events-none">
        <Image
          src="/images/Member/Linesection2.svg"
          alt=""
          width={60}
          height={700}
          className="h-full w-auto opacity-50"
        />
      </div>

      <div className="max-w-360 mx-auto px-20 py-20">
        <div className="grid grid-cols-12 gap-10 items-start">
          <div className="col-span-4 flex flex-col h-full pt-2">
            <h2 className="font-display font-extrabold text-[48px] uppercase leading-[0.95] tracking-[-1.5px] text-black mb-6">
              Beyond the <br />
              <span className="text-volt">Finish Line</span>
            </h2>

            <p className="text-black text-[14px] font-light leading-[1.8] mb-10 max-w-75">
              This collection was born on the training track. Designed for
              athletes who don&apos;t distinguish between training and life.
            </p>

            <div
              key={activeIndex}
              className="border-t border-black/10 pt-6 flex flex-col gap-1"
            >
              <p className="text-black/40 text-[10px] uppercase tracking-[2px]">
                {active.category}
              </p>
              <h3 className="font-display font-bold text-[22px] uppercase text-black leading-tight">
                {active.name}
              </h3>
              <p className="font-display font-bold text-[20px] text-black mt-1 mb-5">
                ${active.price} USD
              </p>
              <button className="flex items-center gap-3 h-11 w-fit px-6 bg-volt text-black font-body font-bold text-[12px] uppercase tracking-[2px] hover:bg-volt-dark hover:text-black transition-colors duration-200 rounded-lg">
                Quick Add
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path
                    d="M2 2H3.5L4.5 10H11.5L13 5H4.5"
                    stroke="currentColor"
                    strokeWidth="1.2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <circle cx="5.5" cy="12" r="0.8" fill="currentColor" />
                  <circle cx="10.5" cy="12" r="0.8" fill="currentColor" />
                </svg>
              </button>
            </div>

            <div className="flex items-center gap-2 mt-8">
              {slides.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setActiveIndex(i)}
                  className="transition-all duration-300 rounded-full"
                  style={{
                    width: i === activeIndex ? "28px" : "6px",
                    height: "2px",
                    backgroundColor:
                      i === activeIndex ? "#0A0A0A" : "rgba(0,0,0,0.2)",
                  }}
                />
              ))}
            </div>
          </div>

          <div className="col-span-8 flex gap-2 h-145">
            {slides.map((slide, i) => {
              const isActive = i === activeIndex;

              return (
                <button
                  key={slide.id}
                  onClick={() => setActiveIndex(i)}
                  className="relative overflow-hidden cursor-pointer shrink-0 group"
                  style={{
                    width: isActive ? "55%" : "15%",
                    transition: "width 0.6s cubic-bezier(0.4, 0, 0.2, 1)",
                    borderRadius: "4px",
                  }}
                >
                  <Image
                    src={slide.image}
                    alt={slide.name}
                    fill
                    sizes="40vw"
                    className="object-cover transition-transform duration-700 group-hover:scale-105"
                    style={{
                      objectPosition: isActive ? "center" : "top center",
                    }}
                  />

                  <div
                    className="absolute inset-0 transition-opacity duration-500"
                    style={{
                      background: isActive
                        ? "linear-gradient(to top, rgba(0,0,0,0.8) 0%, rgba(0,0,0,0.1) 50%, transparent 100%)"
                        : "linear-gradient(to top, rgba(0,0,0,0.7) 0%, rgba(0,0,0,0.4) 100%)",
                    }}
                  />

                  {isActive && (
                    <div className="absolute bottom-6 left-6 z-10">
                      <h3 className="font-display font-extrabold text-[32px] uppercase text-white leading-none tracking-[-1px]">
                        {slide.season} <br />
                        <span className="text-volt">{slide.seasonAccent}</span>
                      </h3>
                    </div>
                  )}

                  {!isActive && (
                    <div className="absolute bottom-6 left-0 right-0 flex justify-center z-10">
                      <p
                        className="font-display font-bold text-[11px] uppercase text-white/60 tracking-[3px]"
                        style={{
                          writingMode: "vertical-rl",
                          transform: "rotate(180deg)",
                        }}
                      >
                        {slide.season} {slide.seasonAccent}
                      </p>
                    </div>
                  )}

                  <div className="absolute top-4 right-4 z-10">
                    <span className="font-display font-bold text-[11px] text-white/40 tracking-[2px]">
                      0{i + 1}
                    </span>
                  </div>
                </button>
              );
            })}
          </div>
        </div>
      </div>
    </section>
  );
}
